...
**Модель**: используется градиентный бустинг `CatBoost`
...
- Текстовые (Nomic): эмбеддинги из модели `nomic-ai/nomic-embed-text-v1.5`.
...
│   ├── features.py       # Feature engineering (агрегаты, жанры, TF-IDF, Nomic)
...
**Кэширование**. Nomic-эмбеддинги и обработанные данные сохраняются в `output/models/` и `data/processed/` для ускорения повторных запусков.
...
- Python >= 3.10
- pandas, scikit-learn, catboost, joblib
